...
- (void)applyLayout {
  if (!self.view.isYogaEnabled) return;
...
- (void)async_applyLayoutPreservingOrigin:(BOOL)preserveOrigin{
    if (!self.view.isYogaEnabled) return;
    [self async_calculateLayoutWithSize:self.view.bounds.size complete:^{
        YGApplyLayoutToViewHierarchy(self.view, preserveOrigin);
    }];
}
...
- (void)async_applyLayoutPreservingOrigin:(BOOL)preserveOrigin dimensionFlexibility:(YGDimensionFlexibility)dimensionFlexibility{
    if (!self.view.isYogaEnabled) return;
    CGSize size = self.view.bounds.size;
    if (dimensionFlexibility & YGDimensionFlexibilityFlexibleWidth) {
      size.width = YGUndefined;
    ...
    [self async_calculateLayoutWithSize:size complete:^{
        YGApplyLayoutToViewHierarchy(self.view, preserveOrigin);
    }];
}
...
- (CGSize)calculateLayoutWithSize:(CGSize)size {
  ...
  if (!self.view.isYogaEnabled) return CGSizeZero;
...
- (void)async_calculateLayoutWithSize:(CGSize)size complete:(dispatch_block_t)complete{
    ...
    YGAttachNodesFromViewHierachy(self.view);
    [DRFlexLayoutTransaction addTransaction:^{
...
static YGSize YGMeasureView(
  ...
  if (!view.yoga.isUIView || [view.subviews count] > 0) {
      if (pthread_main_np() != 0) {
          ...
          __block CGSize _sizeThatFits = CGSizeZero;
          dispatch_sync(dispatch_get_main_queue(), ^{
              _sizeThatFits = [view sizeThatFits:(CGSize){
                                                    .width = constrainedWidth,
                                                    .height = constrainedHeight,
                                                }];
          });
...
static void YGApplyLayoutToViewHierarchy(UIView* view, BOOL preserveOrigin) {
  ...
  for (YGLayoutFinishBlock layoutFinish in view.layoutFinishBlockList) {
      layoutFinish(view);
  }
  for (YGLayoutFinishBlock layoutFinish in view.layoutFinishBlockMap.allValues) {
      layoutFinish(view);
  }

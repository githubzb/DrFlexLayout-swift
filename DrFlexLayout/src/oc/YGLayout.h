...
/**
 ...
 layout results will applied from {0,0}. Synchronous calculate.
 */
...
/**
 Perform a layout calculation and update the frames of the views in the
 hierarchy with the results. If the origin is not preserved, the root view's
 layout results will applied from {0,0}. Asynchronous calculate.
 */
- (void)async_applyLayoutPreservingOrigin:(BOOL)preserveOrigin
    NS_SWIFT_NAME(async_applyLayout(preservingOrigin:));
...
- (void)async_applyLayoutPreservingOrigin:(BOOL)preserveOrigin
                     dimensionFlexibility:(YGDimensionFlexibility)dimensionFlexibility
    NS_SWIFT_NAME(async_applyLayout(preservingOrigin:dimensionFlexibility:));
...
/// Remove child node
- (void)removeChild:(YGLayout *)child NS_SWIFT_NAME(remove(child:));
